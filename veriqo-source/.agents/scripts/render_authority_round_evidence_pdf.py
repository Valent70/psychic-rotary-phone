"""Render the Authority Round: Evidence Trust Bypass Closure report to a
dependency-free, text-based PDF -- same renderer as
render_red_flag_response_pdf.py and its predecessors, pointed at the new
source/output paths. This report's source markdown uses colored-circle
emoji and Unicode arrows/symbols, neither of which the base Helvetica
font (WinAnsi/Latin-1) can render -- so both are substituted for
plain-ASCII equivalents before layout, rather than being silently
dropped or replaced with '?' by the latin-1 encode step."""
...
SOURCE = ROOT / "docs" / "VERIQO_AUTHORITY_ROUND_EVIDENCE_TRUST_BYPASS_RESPONSE.md"
OUTPUT = ROOT / "docs" / "VERIQO_AUTHORITY_ROUND_EVIDENCE_TRUST_BYPASS_RESPONSE.pdf"
