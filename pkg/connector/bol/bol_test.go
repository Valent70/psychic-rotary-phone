...
// IssueDateUTC (fixtures start at 2026-01-01 and run for hours).
const testReceivedAt = 1800000000

func TestDecode_ValidFixtureRoundtrips(t *testing.T) {
	recs := buildFixtureRecords(1, 5)
	c := Contract{Source: "bol-test"}
	for _, rec := range recs {
		ev, err := c.Decode(rec.Payload, testReceivedAt)
		if err != nil {
			t.Fatalf("Decode valid fixture: %v", err)
		}
		if ev.Type != ontology.TypeTradeRecord {
			t.Fatalf("expected TypeTradeRecord, got %s", ev.Type)
		}
		if ev.Attributes["instrument"] == "" {
			t.Fatal("instrument attribute must be populated (ontology's own required-attribute rule)")
		}
	}
}
...
func TestDecode_WrongSchemaFailsClosed(t *testing.T) {
	...
	if _, err := c.Decode(FixtureWrongSchema(), testReceivedAt); !errors.Is(err, ErrMalformed) {
		t.Fatalf("expected ErrMalformed for a SAR-shaped payload fed to the BoL decoder, got %v", err)
	}
}
...
func TestDecode_MissingRequiredFieldFailsClosed(t *testing.T) {
	...
	if _, err := c.Decode(FixtureMissingRequiredField(), testReceivedAt); !errors.Is(err, ErrMalformed) {
		t.Fatalf("expected ErrMalformed for missing cargo_description, got %v", err)
	}
}
...
func TestDecode_SemanticallyInvalidFailsClosed(t *testing.T) {
	...
	if _, err := c.Decode(FixtureSemanticallyInvalid(), testReceivedAt); !errors.Is(err, ErrSemanticallyInvalid) {
		t.Fatalf("expected ErrSemanticallyInvalid for unknown instrument, got %v", err)
	}
}

func TestDecode_BadContainerNumberFailsClosed(t *testing.T) {
	c := Contract{Source: "bol-test"}
	if _, err := c.Decode(FixtureBadContainerNumber(), testReceivedAt); !errors.Is(err, ErrSemanticallyInvalid) {
		t.Fatalf("expected ErrSemanticallyInvalid for malformed container number, got %v", err)
	}
}
