...
| Container digest / base-image SBOM / OS package inventory | PARTIAL | Real `docker build` executed in a sandbox with a working local Docker daemon, for the final `FROM scratch` stages: binaries compiled locally (go1.24.7, `CGO_ENABLED=0 -trimpath -ldflags="-s -w"`, matching the Dockerfile's build-stage flags and satisfying its `go 1.22.2` module directive) were packaged and both `veriqo-demo` and `veriqo-node` images ran to completion (demo: 8/8 workflow steps, decision issued, IVF certificate verified; node: started and stayed up). This run caught and fixed a real bug — `FROM scratch` has no `/tmp`, so `os.TempDir()`-based bundle writing failed until `WORKDIR /tmp` was added. Confirmed zero OS packages: `docker run --entrypoint /bin/sh` fails with "no such file or directory" — no shell exists in either image. Real content-addressed image digests: `veriqo-demo@sha256:42665a6a5121b256e4b4a9334b2fd7c1c7d233b3cc051003c38b7910d6da245c`, `veriqo-node@sha256:9287a4376b657ae38033be7185eff788fc462dc062ead34694ec481da40ad621` (local-image digests; reproduce with the `Dockerfile`'s own `veriqo-demo`/`veriqo-node` targets once a `golang:1.22-alpine` pull succeeds, or by pre-building the binaries and copying them into a scratch-only Dockerfile as done here). Still OPEN: the `golang:1.22-alpine` **build stage** itself was not executed end-to-end in that sandbox — pulling it from Docker Hub is blocked there by a confirmed, explicit organization egress-policy denial (`production.cloudfront.docker.com` returns 403 to CONNECT), not a technical or credentials gap, and it is expected to work unmodified wherever normal registry access exists (e.g. CI). Base-image OS-package SBOM (e.g. `apk info`/`syft` against the alpine layer) is consequently still OPEN — it depends on that same blocked pull. |
