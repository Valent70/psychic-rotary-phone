...
| `scale_qualification` | CODE_COMPLETE | CODE_COMPLETE | CODE_COMPLETE | **WAITING_CLOUD_EXECUTION** | CODE_COMPLETE | EVIDENCE_PENDING | **WAITING_CLOUD_EXECUTION** | Provision 100 real nodes (cloud or physical) and run `pkg/blockers/scale.RunQualification` with `HTTPNodeProvider` at 100 nodes / 1,000,000 envelopes. |
...
- 10 containers cannot become 100-node `VERIFIED`
  (`TestTenContainersCannotBecome100NodeVerified`).
